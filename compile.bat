...
chcp 65001 >nul
:: compile.bat - 支援靈活的項目結構
:: Usage: compile.bat "ProjectPath"
:: Example: compile.bat "LinearAlgebra/Note"     -> 編譯 Note 項目 LinearAlgebra/Note/master.tex
:: Example: compile.bat "LinearAlgebra/A/B/C"   -> 編譯 HW 項目 LinearAlgebra/A/B/C.tex

echo input argument: '%1'
if "%~1"=="" (
    echo Usage: compile.bat "ProjectPath"
    echo.
    echo Examples:
    echo   compile.bat "LinearAlgebra/Note"        ^(Note project^)
    echo   compile.bat "LinearAlgebra/A/B/C"       ^(HW project^)
...
biber --output-directory "%BUILD_DIR%" "%BASE_NAME%"
if errorlevel 1 (
    echo Biber failed, continuing anyway...
)
